                ...
                <div class="sheet" :style="{ gridAutoRows: '24px ' + ('150px ').repeat(parts.length) }">
                    ...
                    <div class="part" v-for="part, i in parts" :key="i" :style="{}">
                        ...
                        <div class="edit" :class="{ 'midi': part.type == 'midi', 'audio': part.type == 'audio' }">
                            <div class="clip" v-for="clip, i in part.content" :key="i"
                                :style="{ width: (150 * zoom.x * clip.duration + 'px '), left: (clip.location * 150 * zoom.x) + 'px' }">
                                <div class="clip-grap"></div>
                                <div class="midi-content" v-if="part.type == 'midi' && 'notes' in clip">
                                    <div v-for="note, j in clip.notes" :key="j" class="note"
                                        :style="{ width: note.duration * 150 * zoom.x + 'px', left: note.location * 150 * zoom.x + 'px' }">
                                    </div>
                                </div>
                            </div>
                        </div>
...
parts.value.forEach(p => {
    if (p.type == "midi") {
        let pitches = p.content.flatMap(c => c.notes.map(n => n.pitch));
        let min = pitches[0], max = pitches[0];
