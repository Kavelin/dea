...
export let grid = (e: MouseEvent, t: string, mod:Modify) => {
    if (t == "down" && !mod.pan.moving) {
      mod.pan.moving = true;
      mod.pan.offset.x = e.clientX - (<HTMLDivElement>e.target).clientLeft - mod.pan.x;
      mod.pan.offset.y = e.clientY - (<HTMLDivElement>e.target).clientTop - mod.pan.y;
    }
  ...
  export let node = (e: MouseEvent, t: string, node:Node) => {
    if (t == "down" && !node.moving) {
      node.moving = true;
      node.mOffset.x = e.clientX - (<HTMLDivElement>e.target).parentElement!.parentElement!.clientLeft - node.x;
      node.mOffset.y = e.clientY - (<HTMLDivElement>e.target).parentElement!.parentElement!.clientTop  - node.y;
    }
